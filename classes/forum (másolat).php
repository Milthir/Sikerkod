...
class forum
{
	...
	var $admin_mode;//ha ez bevan állitva akkor mindenhez van jog

	var $topic_id;//aktuális topic id
	var $cat_id;//aktuális kategória id
	var $reply_to_id;//aktuális post amire válaszol

	var $akt_user;//aktuális user
	var $akt_privs;//aktuális user aktuális kategóriára nézett jogai
	...
	//funkciók
	var $UserNameFunc; //ezzel kéri le a usernevet
	var $MessageFunc; //üzenet küldése kifelé
	...
	function forum($akt_user,$admin_mode=false,$set_def_templates=true)
	{
		$this->prefix="forum_";
		...
		$this->getUserName = create_function('$id','return "";');
		//$this->MessageFunc = create_function('$id,$text','return "";');


		$this->forum_text = array(
		...
		if($set_def_templates)
		{
			...
			$this->templates[FORUM_TEMPLATE_POSTVIEW] = new template('<div class="forum_postlist" style="border:2px solid;">{pager}{body}{pager}<hr /><form method="post" action="">[if {reply_to_id}!=0]Válasz {reply_to} üzenetére[/if]{hidden_zone}<textarea name="{textarea}" rows="10" cols="30"></textarea><input type="submit" /></form></div>');

			$this->templates[FORUM_TEMPLATE_CATVIEW] = new template('<div class="forum_catlist" style="border:2px solid;">KATEGÓRIÁK<br />{cat_pager}{cat_body}{cat_pager}<hr />{new_cat_form}</div><div class="forum_topiclist" style="border:2px solid;">TOPIKOK<br />{topic_pager}{topic_body}{topic_pager}<hr />{new_topic_form}</div>');
		...
		//	$this->templates[FORUM_TEMPLATE_PRIVPAGE] = new template('<fieldset><legend>Jogok módositása</legend></fieldset>');
		}
	}

	function setUserNameFunc($code)
	{
		$this->UserNameFunc = $code;
	}
	function setMessageFunc($code)
	{
		//$this->MessageFunc = create_function('$id,$text',$code);
		$this->MessageFunc = $code;
	}
	...
	//TEMPLATE START// 
	//////////////////
	//Első paraméter az előre definiált makrók a 2. meg maga a template osztály
	...
	function isHavePriv($priv,$cat=-1,$user=-1)
	{
		...
		{
			$is_true = selectMezo("SELECT `privs_privs` FROM `".$this->prefix."privs` WHERE `privs_user_id`=".($user?$user:$this->akt_user)." AND ".
																	  "`privs_cat_id`=".($cat?$cat:$this->cat_id)   ,'privs_privs')&$priv;
	...
	{
		if(isNumber($id))return selectMezo("SELECT `topic_cat` FROM `".$this->prefix."topic` WHERE `topic_id`=".$id,"topic_cat");
		return 0;
	}
	function getCatFromPost($id)
	{
		if(isNumber($id))return $this->getCatFromTopic(selectMezo("SELECT `post_topic` FROM `".$this->prefix."post` WHERE `post_id`=".$id,"post_topic"));
		return 0;
	}
	...
	function getPosition($id,$first=true)
	{
		...
		else $str .= '<a href="'.delGet(array($this->cat_get,$this->topic_get,"rt")).'">Gyökér</a>';
