...
class template 
{
	...
	function parseControls($input)
	{
		...
		if (is_array($input)) // 1-es művelet 2-es a művelet paramétere 3-as okés ág 4-es else ág
		{
			...
			{     			
				case 'if':
					$ret = @eval("return (".($this->parseVarsFunctions($input[2])).")?1:0;");		
					if($ret === false)
					{
						report_error("template::rossz kifejezés:".listArray($input,false));
						return preg_replace_callback($regex, array(&$this,'parseControls'), $input[4]);
					}
					else if($ret) return preg_replace_callback($regex, array(&$this,'parseControls'), $input[3]);
					else return preg_replace_callback($regex, array(&$this,'parseControls'), $input[4]);
					break;
				...
				case 'repeat':
					...
					else return preg_replace_callback($regex, array(&$this,'parseControls'), str_repeat($input[3],$ret));
					break;
			}
		}
		else return preg_replace_callback($regex, array(&$this,'parseControls'), $input);
	}
