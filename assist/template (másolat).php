...
*/
	...
	{//átkell csinálni callback-esre ezt a részt is
	/*	$temp = $this->text;
		foreach($this->classes as $key => $val)
		{
			while(preg_match("/{".$key."->([^(]+)\(([^)]*)\)}/",$temp,$reg))
			{
				$temp = str_replace($reg[0], eval("return \$this->classes['".$key."']->".$reg[1]."(".$reg[2].");"),$temp);
			}
		}
		...
		foreach($this->functions as $key => $val)
		{
			while(preg_match("/{".$key."\(([^)]*)\)}/",$temp,$reg))
			{
				$temp = str_replace($reg[0], eval("return \$this->functions['".$key."'](".$reg[1].");"),$temp);
			}
		}
		...
		foreach($ar as $key => $val)
		{
			while(preg_match("/{".$key.":([^}]+)}/",$temp,$reg))
			{
				$temp = str_replace($reg[0],date($reg[1],$val),$temp);
			}
